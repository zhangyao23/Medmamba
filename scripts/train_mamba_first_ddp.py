...
class Volumetric3DMIL(nn.Module):
    def __init__(self, config):
        ...
        if self.codebook_after_mamba:
            self.mamba_out_norm = nn.LayerNorm(feature_dim)
    ...
    def forward(self, patches, coords, labels=None, masks=None, mini_batch_size=8):
        ...
        if self.codebook_after_mamba:
            ...
            z_e = context_orig
            context_normed = self.mamba_out_norm(context_orig)
            quantized, codes, vq_loss = self.codebook(context_normed, labels)
        ...
        distill_feat = features if (self.training and self.codebook_after_mamba) else (context_orig if self.training else None)
        return logits, attention, vq_loss, codes, sorted_coords, correction_rate, recon_patches, distill_feat, patch_seg_logits, voxel_seg_logits, code_cls_logit, embed_sep_loss, pseudo_routing_loss, patch_cancer_logits, mil_seg_logits, z_e_frozen
...
def train_one_epoch(model, dataloader, optimizer, loss_fn, epoch, device, rank, config,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        try:
            ...
            if train_mode == 'mil_train_phase2' and rank == 0 and batch_idx < 15:
                nan_modules = []
                for name, p in model.named_parameters():
                    if p.requires_grad and torch.isnan(p).any():
                        nan_modules.append(name.split('.')[1] if '.' in name else name)
                        break
                if nan_modules:
                    logging.info(f"[Phase2 dbg] batch={batch_idx} NaN in param: {nan_modules[0]} scale={scaler.get_scale():.1f}")
...
def set_trainable_for_pretrain(model_module, pretrain_enabled):
    include_mamba = getattr(model_module, 'codebook_after_mamba', False)
    if include_mamba:
