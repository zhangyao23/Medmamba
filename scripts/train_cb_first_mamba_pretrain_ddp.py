...
class Volumetric3DMIL(nn.Module):
    ...
    def forward(self, patches, coords, labels=None, masks=None, mini_batch_size=8):
        ...
        if self.codebook_after_mamba:
            ...
            recon_patches = None
            if self.decoder_enabled:
                quantized_for_recon = quantized * 0.1 + quantized.detach() * 0.9
                recon_patches = self.decoder(quantized_for_recon)
            ...
            if self.decoder_enabled:
                if self._pretrain_mode:
                    recon_patches = self.decoder(context_orig)
                else:
                    quantized_for_recon = quantized * 0.1 + quantized.detach() * 0.9
                    recon_patches = self.decoder(quantized_for_recon)
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        try:
            ...
            if oom_flag.item() > 0:
                oom_skip_count += 1
                if rank == 0:
                    logging.warning(f"OOM on some rank at batch {batch_idx}, all ranks skip (total skipped: {oom_skip_count})")
                optimizer.zero_grad(set_to_none=True)
            ...
            optimizer.zero_grad(set_to_none=True)
            torch.cuda.empty_cache()
            oom_flag = torch.ones(1, device=device)
            dist.all_reduce(oom_flag, op=dist.ReduceOp.MAX)
            continue
...
def set_trainable_for_pretrain(model_module, pretrain_enabled, include_mamba_in_pretrain=False):
    if include_mamba_in_pretrain and pretrain_enabled:
        for p in model_module.mamba.parameters():
            p.requires_grad = True
        for p in model_module.spatial_scanner.parameters():
            p.requires_grad = True
    else:
        for p in model_module.mamba.parameters():
...
def main():
    ...
    config.training['_grad_scaler'] = torch.cuda.amp.GradScaler(enabled=config.training.get('mixed_precision', False))
    ...
    if config.training.get('phase2_freeze_recon_pipeline', False):
        loss_fn.phase2_disable_recon = False
    voxel_seg_config = config.model.get('voxel_seg', {})
    ...
    include_mamba_in_pretrain = config.training.get('pretrain_include_mamba', False)

    if pretrain_loader is not None and start_epoch == 0:
        if rank == 0:
            logging.info("=" * 60)
            logging.info("PHASE 0: HEALTHY-ONLY VQ-VAE PRETRAIN")
            if include_mamba_in_pretrain:
                logging.info("Mamba INCLUDED in Phase 0 pretrain (Decoder receives Mamba output)")
            logging.info("=" * 60)
        model.module._pretrain_mode = include_mamba_in_pretrain
        set_trainable_for_pretrain(model.module, pretrain_enabled=True, include_mamba_in_pretrain=include_mamba_in_pretrain)
        optimizer = rebuild_optimizer(model, config)
        ...
        model.module._pretrain_mode = False
        set_trainable_for_pretrain(model.module, pretrain_enabled=False)
        optimizer = rebuild_optimizer(model, config)
        if rank == 0:
            logging.info("Phase 0 ended, _pretrain_mode=False, Mamba frozen for Phase 1+")
    ...
    for epoch in range(start_epoch, config.training['epochs']):
        ...
        if use_two_phase and epoch >= phase1_epochs:
            phase2_lr_warmup_epochs = int(config.training.get('phase2_lr_warmup_epochs', 3))
            phase2_lr_start_factor = float(config.training.get('phase2_lr_start_factor', 0.1))
            phase2_epoch = epoch - phase1_epochs
            if phase2_lr_warmup_epochs > 0 and phase2_epoch < phase2_lr_warmup_epochs:
                warmup_factor = phase2_lr_start_factor + (1.0 - phase2_lr_start_factor) * (phase2_epoch / phase2_lr_warmup_epochs)
                epoch_lr = epoch_lr * warmup_factor
                cancer_logit_lr = cancer_logit_lr * warmup_factor
                mil_seg_lr = mil_seg_lr * warmup_factor
                if rank == 0 and epoch == phase1_epochs:
                    logging.info(f"Phase 2 LR warmup: {phase2_lr_warmup_epochs} epochs, start_factor={phase2_lr_start_factor}")
        ...
        if use_two_phase and epoch == phase1_epochs and (not model.module.codebook.phase1_complete):
            ...
            if phase2_freeze_recon_pipeline:
                ...
                for p in model.module.decoder.parameters():
                    p.requires_grad = True
                model.module.codebook.ema_frozen = False
                optimizer = rebuild_optimizer(model, config)
                if rank == 0:
                    if phase2_unfreeze_encoder:
                        enc_lr = config.training.get('encoder_lr', config.training['lr'])
                        logging.info(f"Phase 2: ENCODER UNFROZEN with distillation (lr={enc_lr})")
                    logging.info("Phase 2: DECODER UNFROZEN, CODEBOOK EMA UNFROZEN (recon continues)")
            ...
            config.training['_grad_scaler'] = torch.cuda.amp.GradScaler(enabled=config.training.get('mixed_precision', False))
            if rank == 0:
                logging.info("Phase 2: GradScaler reset")
            dist.barrier(device_ids=[rank])
            sync_codebook_state(model.module)
            dist.barrier(device_ids=[rank])
